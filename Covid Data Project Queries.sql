...
------------------- GARRET'S ANALYSIS PREP ------------------------------

-- Total cases by country - 04/30/2021 --- 4 separate ways to filter 
SELECT location, total_cases
FROM public."CovidDeaths"
WHERE (date = '2021-04-30' AND total_cases IS NOT NULL) 
	 AND location NOT IN('World', 'Europe', 'Asia', 'North America', 'European Union', 'South America', 'Africa', 'Oceania')
ORDER BY total_cases DESC;

SELECT location, MAX(total_cases) AS total_cases_maxed
FROM public."CovidDeaths"
WHERE total_cases IS NOT NULL 
	 AND location NOT IN('World', 'Europe', 'Asia', 'North America', 'European Union', 'South America', 'Africa', 'Oceania')
GROUP BY location
ORDER BY total_cases_maxed DESC;
...
-- Total vaccinations by country - MAX date -- 
